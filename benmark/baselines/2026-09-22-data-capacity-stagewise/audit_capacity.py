...
COLLECTOR_FILES = ("benmark/experiment.py", "benmark/summarize.py",
                   "benmark/experiment_support.py", "benmark/snapshot_report.py", "benmark/process_guard.py")
COLLECTOR_REVISION = "bc7e031a962ff44367a160a5fc5adcda83ac1893"
EXECUTION_PROTOCOL = "single-stage-v1"
...
def validate_plan(root, plan):
    ...
    require(plan.get("execution_protocol") == EXECUTION_PROTOCOL, "unexpected execution protocol")
    require(plan.get("collector_revision") == COLLECTOR_REVISION, "unexpected collector revision")
    require(isinstance(plan.get("collector_files_sha256"), dict)
            and set(plan["collector_files_sha256"]) == set(COLLECTOR_FILES), "collector requires all five frozen modules")
    require(all(isinstance(value, str) and re.fullmatch("[0-9a-f]{64}", value)
                for value in plan["collector_files_sha256"].values()), "invalid collector digest")
    require(plan["policies"].get("stop_after_failed_stage") is True, "single-stage protocol must stop after failure")
...
def process_identity(identity, label, timestamp):
    require(isinstance(identity, dict), "missing " + label + " process identity")
    require(all(type(identity.get(key)) is int and identity[key] > 0 for key in ("pid", "ppid")),
            "invalid " + label + " PID")
    require(uint(identity.get("starttime_ticks")), "invalid " + label + " process start time")
    require(isinstance(identity.get("boot_id"), str)
            and re.fullmatch(r"[0-9a-f]{8}(?:-[0-9a-f]{4}){3}-[0-9a-f]{12}", identity["boot_id"]),
            "invalid " + label + " boot identity")
    require(isinstance(identity.get("exe"), str) and "\0" not in identity["exe"]
            and Path(identity["exe"]).is_absolute(), "invalid " + label + " executable identity")
    require(isinstance(identity.get("cmdline"), list) and identity["cmdline"]
            and all(isinstance(word, str) and "\0" not in word for word in identity["cmdline"]),
            "invalid " + label + " command identity")
    require(isinstance(identity.get("state"), str) and re.fullmatch("[A-Za-z]", identity["state"]),
            "invalid " + label + " process state")
    return timestamp(identity["observed_at"])


def check_execution_guard(record, plan, timestamp):
    controller = record.get("controller")
    controller_time = process_identity(controller, "controller", timestamp)
    command, launcher = record.get("argv"), record.get("launcher")
    require(isinstance(command, list) and len(command) >= 2
            and all(isinstance(word, str) and "\0" not in word for word in command)
            and Path(command[0]).is_absolute() and Path(command[1]).is_absolute()
            and command[1].endswith("/benmark/experiment.py"), "invalid recorded runner command")
    require(isinstance(launcher, dict) and set(launcher) == {"parent_pid", "argv_prefix"}
            and type(launcher["parent_pid"]) is int and launcher["parent_pid"] == controller["pid"],
            "outer launcher parent differs from controller PID")
    expected = [command[0], "-I", "-S", str(Path(command[1]).with_name("process_guard.py")), str(controller["pid"])]
    require(launcher["argv_prefix"] == expected, "outer process guard prefix differs")
    require(record.get("launcher_sha256") == plan["collector_files_sha256"]["benmark/process_guard.py"],
            "outer process guard hash differs from frozen helper")
    runner = record.get("runner")
    if runner is None:
        require(record.get("finished_at") is None and record.get("returncode") is None,
                "finished stage lacks runner identity")
        return
    runner_time = process_identity(runner, "runner", timestamp)
    require(runner["pid"] != controller["pid"] and runner["ppid"] == controller["pid"]
            and runner["boot_id"] == controller["boot_id"], "runner is not the recorded controller child")
    require(runner["cmdline"] in (command, expected + command), "observed runner command differs from launch")
    require(controller_time <= runner_time and timestamp(record["started_at"]) <= runner_time,
            "runner observation precedes controller/stage")
    if record.get("finished_at") is not None:
        require(runner_time <= timestamp(record["finished_at"]), "runner observation follows stage completion")
...
def execution_evidence(root, plan, timestamp):
    result = {"errors": [], "warnings": [], "complete": False, "recorded_status": None,
              "stages": {stage["directory"]: {"errors": [], "warnings": [], "record": None,
                                                "state": "missing", "completion_verified": False}
                         for stage in plan["stages"]}}
    try:
        execution = load(root / "execution.json")
        result["recorded_status"] = execution.get("status")
        require(execution.get("execution_protocol") == EXECUTION_PROTOCOL, "execution protocol differs from plan")
        ...
        for position, record in enumerate(records):
            ...
            item["record"] = record
            if not may_continue:
                item["errors"].append("stage started after a failed or unverified predecessor")
            try:
                check_execution_guard(record, plan, timestamp)
            except Exception as error:
                item["errors"].append(str(error))
            try:
                require(isinstance(record.get("errors"), list)
                        and all(isinstance(error, str) for error in record["errors"]), "invalid stage error evidence")
                if record["errors"]:
                    item["warnings"].extend("controller: " + error for error in record["errors"])
                if record.get("finished_at") is not None:
                    cleanup = record.get("cleanup")
                    require(isinstance(cleanup, dict) and cleanup.get("status") == "verified_absent"
                            and cleanup.get("live_processes") == [] and isinstance(cleanup.get("actions"), list),
                            "finished stage lacks verified process cleanup")
                    require(timestamp(record["started_at"]) <= timestamp(cleanup["checked_at"])
                            <= timestamp(record["finished_at"]), "cleanup evidence lies outside stage")
                    item["completion_verified"] = record.get("returncode") == 0 and not record["errors"]
            except Exception as error:
                # Missing/failed stage cleanup does not retroactively erase
                # earlier rounds with their own successful wait/WAL evidence.
                item["warnings"].append("stage completion: " + str(error))
            # Stop policy follows the controller's observed failure/cleanup,
            # not unrelated provenance corruption in an earlier archive row.
            may_continue = item["completion_verified"]
            try:
                ...
                if record.get("finished_at") is None:
                    ...
                    previous_end = last
                    item["state"] = "complete" if item["completion_verified"] else "failed"
        ...
        if execution["status"] == "complete":
            ...
            result["complete"] = all(item["completion_verified"] and not item["errors"]
                                     for item in result["stages"].values())
            if not result["complete"]:
                result["warnings"].append("execution claims completion without every verified stage")
        else:
...
def check_stage(root, plan, stage, manifest, common, execution):
    ...
    guard = manifest.get("process_guard")
    require(isinstance(guard, dict), "single-stage experiment lacks required process guard metadata")
    require(guard.get("sha256") == plan["collector_files_sha256"]["benmark/process_guard.py"],
            "stage process guard differs from frozen collector helper")
    ...
    require(isinstance(execution.get("runner"), dict), "stage artifacts lack recorded runner identity")
    ...
    require(guard["python"] == command[0], "stage guard interpreter differs from runner interpreter")


def check_round_launcher(directory, case, execution):
    require(isinstance(execution, dict) and isinstance(execution.get("runner"), dict),
            "round lacks recorded runner identity")
    commands = load(relative(directory, case["name"] + "/commands.json"))
    launcher = commands.get("launcher")
    require(isinstance(launcher, dict) and type(launcher.get("parent_pid")) is int
            and launcher["parent_pid"] == execution["runner"]["pid"],
            "round process guard parent differs from observed runner PID")
...
def analyze_round(root, plan, stage, case, manifest, base, stage_errors, modules, execution):
    ...
    try:
        ...
        check_round_launcher(directory, case, execution)
...
def main():
    ...
    if not execution["complete"]:
        for group in analysis["comparisons"]:
            group.update(classification="inconclusive", triggered_metrics=[], engine_rss_diagnostic="inconclusive",
                         execution_warning="single-stage execution is incomplete or lacks verified cleanup")
