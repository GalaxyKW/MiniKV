...
TCP 服务使用 `Engine::execute_async`。数据线程应用请求并保存响应结果；如果目标序列尚未持久化，就登记一个等待项并返回，继续执行其他任务。独立完成线程在目标提交后调用回调，回调将结果放入有界完成队列，通过 `eventfd` 通知 reactor 发送。WAL 容量不足仍会阻塞数据线程；这次异步化只改变可靠确认的等待位置。
