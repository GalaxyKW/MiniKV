...
`EngineConfig::max_data_bytes`（环境变量 `MINIKV_MAX_DATA_BYTES`）可限制当前映射中 key/value 字节数之和，默认 0 不限额。`data_bytes` 随内存更新在状态锁内维护，普通操作只计算该 key 的变化量，不扫描整个数据集。已有 key 的 PUT 用新值替换旧值的字节数；新 key 同时计算 key 和 value；DELETE 命中释放两者，未命中不改变数据量。

PUT 超出正上限时返回 BUSY，增加 `data_rejections_total`，不修改映射、日志队列或序列，也不将存储标为失效。检查与操作应用使用同一状态锁；若请求等待 WAL 额度时释放了锁，重新取得锁后必须按最新映射再次检查，不能沿用等待前的额度或旧值大小。同步和异步接口使用同一规则；异步即时拒绝不接管回调，预留名额随调用返回释放。DELETE 仍按原有规则写日志和等待持久化，不会因数据集已满而拒绝。

启动完成快照和 WAL 回放后，统一计算最终数据量并检查上限，再允许尾部修复或旧格式升级；文本导入在安装新文件前执行同样检查。历史快照或中间日志状态可能比最终数据集大，因此不在中间状态提前拒绝。超限只导致本次打开失败，不会删除数据来凑足额度，也不修改原数据文件内容。数据量从文件重建，拒绝计数从零开始，容量取本次启动配置；无需改变磁盘格式。

这里限制的是逻辑数据量，既不等于分配器实际占用，也不包括快照副本、请求和日志缓冲。恢复过程中仍可能加载较大的历史状态；这项配置不是进程内存硬限制或磁盘空间限制。

## WAL 和快照文件

文件名保留为 `wal.v1`、`snapshot.v1`；内容版本由 magic 标识，当前写入 WAL v2，快照仍使用 v1。`LOCK` 使用 `flock` 阻止两个新版引擎同时打开同一目录。新建数据文件权限为 0600；数据目录支持配置。

引擎在启动时打开数据目录，随后通过同一个目录描述符执行 `openat`、`renameat` 和目录 `fsync`，关闭时才释放。数据目录本身可以是符号链接；运行中重定向该链接，或重命名目录后复用原路径，都不会把快照、WAL 或同步操作转移到另一个目录。路径中的符号链接和 `..` 按文件系统语义解析，不提前做纯字符串折叠。

打开 `LOCK`、WAL、快照和待导入的旧文件时，拒绝文件本身的符号链接及非普通文件，包括悬空链接、FIFO 和目录。FIFO 使用非阻塞方式打开后检查类型，避免启动一直等待。可修改的 `LOCK` 与 WAL 还必须只有一个硬链接，防止独立目录锁下的数据库共享可写 inode；只读的快照及旧文件允许硬链接，后续快照通过替换文件安装，不修改旧 inode。这些检查依赖数据目录由引擎独占管理，不防止其他进程绕过锁修改文件或删除、替换 `LOCK`。

所有整数均为大端序。WAL v2 以 24 字节文件头开始：`MKVWAL02`（8）、零 flags（4）、零保留字段（8）、前 20 字节的 CRC32（4）。即使没有日志记录也保留文件头；不支持 v2 的旧引擎会拒绝读取，不会将它当作可丢弃的短尾记录。

WAL v2 与快照记录的布局如下：

| 字段 | 字节数 |
| --- | ---: |
| magic：WAL 为 `MKL2`，快照为 `MKL1` | 4 |
| 操作：PUT=1、DELETE=3 | 1 |
| 序列号 | 8 |
| key 长度 | 4 |
| value 长度 | 4 |
| 记录头 CRC32：覆盖前 21 字节，仅 WAL v2 有此字段 | 4 |
| key、value 原始字节 | 可变 |
| CRC32（IEEE，覆盖此前全部记录字节） | 4 |

CRC32 使用 IEEE 多项式。key 为 1–4096 字节，value 为 0–1048576 字节。DELETE 的 value 长度必须为 0。WAL 固定记录头为 25 字节，先校验头部再使用长度字段；长度上限在分配 payload 内存前验证。每条 WAL 记录比旧版多 4 字节，这部分计入未同步队列额度。

快照头为 `MKVSNP01`（8 字节）、快照序列号（8 字节）、条目数（8 字节）及前 24 字节的 CRC32（4 字节）。随后恰有指定数量的 PUT 记录，各记录序列号等于快照序列号。空快照也包含完整的头部。

启动先验证并加载快照，再顺序验证 WAL。快照已覆盖的日志记录不会再次应用；新日志序列必须连续。v2 最后一条记录的固定头未写完，或头部通过校验但正文/尾部 CRC 未写完时，可截断到上一完整记录边界；完整头部校验失败会拒绝启动，不会把损坏的长度字段当作缺失正文。文件头损坏或不完整、完整记录校验错误、序列缺失、快照截断、额外快照字节和缺失 WAL 也会导致启动失败。若 WAL 全部被快照覆盖，恢复时会在同步快照和目录后移除旧记录，保留 24 字节文件头，避免后续追加产生序列间隙。

旧 WAL 没有文件头，使用 `MKL1` 记录。由于其长度字段没有独立校验，一次位翻转就可能把完整记录伪装成不完整尾部。当前版本对旧 WAL 只读验证，任何不完整记录都拒绝自动修复，且不改动原文件。完整有效的旧数据库会在启动工作线程前安装新快照，再按下述原子替换顺序升级 WAL；启动中断后可由已安装快照和旧/新 WAL 继续恢复。升级会写入文件，运行前应停止旧进程并备份整个目录；完成后不能直接用旧二进制降级。

CRC 和连续序列用于识别可观测的损坏，不是外部提交账本：无法识别整条末尾记录被移除或整组文件回滚，也无法区分旧版合法空 WAL 与被整体清空的 WAL。恢复与升级不能找回已经丢失的数据。
...
5. 重新获取 I/O 锁，先向 `wal.v1.tmp` 写入 v2 文件头，再将原 WAL 中 B 之后的日志以固定大小缓冲区复制过去，执行 `fdatasync`。快照已经覆盖的日志不再写入新 WAL。
...
观测分别记录第 1 步捕获临界区的持锁时间、快照文件实际写调用与写入字节、第 4 步成功安装的字节，以及第 5 步后缀重写的字节。文件计数使用阶段局部累加，在现有阶段结算锁中发布；它们不等于磁盘物理写入量，也不改变安装或确认顺序。失败与采样窗口的含义见[指标口径](observability.md#快照写入量)。
...
每次写临时文件前，只删除同名的旧目录项，再用 `O_CREAT | O_EXCL` 创建新文件；不对已有 inode 执行截断。因此残留临时文件即使是符号链接或硬链接，也不会覆盖它指向的正式数据或备份。临时路径若是目录则报错，保留其中内容。启动时如果 WAL 本身已有多个硬链接，会先拒绝打开，不会为清理临时文件而放宽检查。
...
STATS 请求的 key/value 长度必须都为 0。成功响应为 VALUE，payload 是 `schema_version: 1` 的聚合状态 JSON，包含 `engine` 和 `server`；它不属于持久化操作，不能写入 WAL。STATS 扩展不改变 PUT/GET/DELETE 的网络帧；不支持 STATS 的旧引擎会拒绝该操作。网络协议版本与 WAL 内容版本相互独立。
...
非法帧返回 INVALID 后结束连接。此时先半关闭写端，再丢弃后续未接纳输入，保留已经排入 TCP 的响应，包括非法帧之前的有效请求响应；直接关闭带未读输入的连接可能产生 RST 并截断它们。等待已排队响应完成传输仍受空闲超时约束，丢弃输入不会延长这个超时，后续帧也不会执行。
...
RPC 池之外，网关在读取 `/kv` 正文前非阻塞取得 HTTP 接纳名额，默认容量等于 RPC 池，可由 `MINIKV_HTTP_MAX_INFLIGHT` 单独配置。名额覆盖参数解析、正文解码、RPC 帧分配、池等待及 handler 的响应写入，handler 返回或异常退出时归还；请求取消本身不提前归还。这样慢后端不会让任意多的请求携带已解码 value 和 RPC 帧等待连接池。满额返回 HTTP 503，未调用后端；HTTP/1 同时设置 `Connection: close`，防止 net/http 在发出拒绝前等待未读正文。不主动提前关闭请求体，连接清理由 HTTP 读超时约束。该名额不覆盖连接读头、拒绝路径和 handler 返回后的 net/http 收尾，因此不承诺总连接数或 RSS 有界。

`GET /stats` 使用独立的 1 个 HTTP 接纳名额和容量为 1 的网关 RPC 池，数据 HTTP 名额占满时仍可访问；状态 HTTP 名额满时直接返回含本机指标的 `503 gateway_overloaded`。引擎将 STATS 派发到固定 1 个线程、1 个待执行位置的专用任务池，使用同一个 reactor 和完成队列；两个任务池均在关闭完成通知文件描述符之前排空。状态采样在工作线程中获取状态锁，不等待 durable 序列，不在 reactor 上等待锁，也不在取样时持有 I/O 锁。WAL 已失效时仍可读取状态。
