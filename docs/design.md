...
状态锁保护内存数据、日志入队、序列号和等待条件；独立的 I/O 锁串行化 WAL 写入、快照安装和最终关闭。需要两把锁时，固定先取 I/O 锁，再取状态锁；后台线程会先释放状态锁再等待 I/O 锁，防止锁顺序反转。
...
I/O 锁覆盖上面整个过程，所以始终只有一个批次在写盘，快照也不能越过未完成的批次去截断 WAL。WAL 写盘期间，新请求可以修改内存并进入下一批队列。提交点只前进到本批最后一个序列号 S，不能使用此时可能更大的 `applied_sequence`。
...
| 快照 | 先等待 I/O 锁，再锁住状态、同步剩余队列并安装快照 |
| 关闭 | 停止接入，等待正在执行的批次，再同步剩余队列 |

快照制作期间仍持有状态锁，遍历并写出整个内存数据集仍会影响尾延迟。下一步若将快照 I/O 也移出临界区，需要固定快照版本，并保留快照序列之后的 WAL；不能直接沿用当前整文件截断方式。
...
1. 同时持有 I/O 锁和状态锁，同步全部待提交 WAL，并确定快照序列 S。
