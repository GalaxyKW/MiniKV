...
新版网关默认将 HTTP 接纳上限设为 `--rpc-pool` 对应的池大小。实验程序隔离宿主的 `MINIKV_*` 环境，新网关的接纳字段保存在原始 `/stats` 样本的 `gateway.http` 中；旧网关缺少这些字段表示不可用。并发超过上限时，网关可在解析正文前返回 503，这仍是压测失败请求，不能改算成功或与引擎 BUSY 混为一种瓶颈。比较新旧网关时须保留二进制版本及原始样本，结合 HTTP 拒绝计数、RPC 等待和引擎名额判断失败发生在哪一层。
