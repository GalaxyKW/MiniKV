...
[大数据集快照预实验](docs/performance-snapshot-pilot.md)进一步测量了 20k / 100k key、1 KiB value 的四轮负载。100k 自动快照轮次中，写出阶段平均约 636 ms，Engine RSS 样本最大值约 225 MiB，提示应同时关注写出开销和完整状态副本。当前已为快照增加固定 64 KiB 写缓冲并补充部分写失败后的恢复验证；预实验使用缓冲前版本，性能收益仍需新旧版本对照。
...
| P2 | 验证大数据集下的快照优化 | 以已归档预实验为起点，对照写缓冲前后版本；补齐状态锁暂停与写入字节口径，联合检查尾延迟、内存和 WAL 积压 |
