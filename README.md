...
当前面向 **Linux、单机、全量内存数据集**。尚无事务、复制、鉴权、TLS 或内存淘汰。可通过 `MINIKV_MAX_DATA_BYTES` 限制当前 key/value 的总字节数，默认不限；这是逻辑数据量上限，进程内存还包含容器、在途请求和快照副本。
...
完整有效的旧二进制格式数据库启动时会自动升级为具有独立记录头校验的 WAL v2；先停机并备份目录，升级后不能直接使用旧二进制降级。文件名仍为 `wal.v1`，兼容与损坏处理边界见[迁移说明](docs/usage.md#旧版数据迁移)。
...
| 在同一状态锁内检查数据字节净增长 | 超限 PUT 无副作用地拒绝，覆盖缩小与删除释放额度 | 限额默认关闭，不限制进程 RSS；恢复按最终数据集验证，但仍需加载历史状态 |
...
| WAL 记录头与整条记录分别校验，检查连续序列与文件组合 | 避免损坏的长度伪装成可截断尾部，识别关键文件缺失 | v2 可修复不完整尾记录；旧格式含糊的尾部拒绝自动修复 |
...
这一设计把可靠确认等待移出 worker，也引入回调所有权、异常处理和关闭顺序的问题。[生命周期契约](docs/design.md#异步可靠确认与请求生命周期)与[门控测试](tests/async_test.cpp)分别记录并验证这些边界。
...
等待指标显示，可靠确认占用了大部分数据 worker。经过[刷新间隔](docs/performance-controls.md)和[线程数对照](docs/performance-worker-controls.md)，引擎将确认等待交给独立完成线程，同时保留待完成请求的容量限制。

下表比较异步版本 `7d29382` 与随后重新运行的同步版本 `25cec55`，均为 reliable、20 个数据线程。每组 3 轮，数值为轮次中位数；这 12 轮全部完成，系统失败为 0。

| 自动快照 | 成功 QPS：同步返回对照 → 异步 | P99：同步返回对照 → 异步 |
| --- | ---: | ---: |
| 关闭 | 5,128 → 9,616 | 10.026 → 5.907 ms |
| 1,000 ms | 5,103 → 9,541 | 11.251 → 7.253 ms |

关闭快照时，活动数据线程的样本均值中位数由 19.74 降到 0.55，与可靠确认等待迁出 worker 的设计一致；持久化承诺保持不变。

条件为 Xeon E3-1270 v3、Linux/NTFS3、客户端与服务端共用主机；40 个闭环客户端，每轮 100,000 请求、5,000 key、128 字节 value，PUT/DELETE/GET 为 20%/5%/75%，RPC 池 64、WAL batch 64、flush 2 ms。成功 QPS 包含正常 `NOT_FOUND`。执行顺序未随机交错，结果仅覆盖所列历史版本和负载，不是容量承诺。采样口径、每轮资源数据与原始归档见[完整报告](docs/performance-async-controls.md)。

其余实验保留局部收益和负向结果：

| 方向 | 当前证据边界 | 报告与原始记录 |
| --- | --- | --- |
| throughput 请求路径简化 | 减少无业务工作的线程与回调构造；交错对照未观察到一致的端到端收益 | [引擎交错对照](docs/performance-throughput-interleaved.md) |
...
| 优先级 | 方向 | 验收条件 |
| --- | --- | --- |
| P1 | 扩大异步确认的负载验证 | 在已归档的固定到达率名额对照基础上，扩大新旧版本交错实验、延长运行并加入实际慢存储；联合检查丢弃率、失败率、计划到达至完成的 P99、请求名额与内存 |
