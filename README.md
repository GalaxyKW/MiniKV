...
初始基线保存了提交 `7c488b7` 的 **24 轮原始实验记录**：两种 WAL 模式 × 快照开关 × 3 次重复，分别开启和关闭周期采样。以下为开启采样的 12 轮结果，每组列出轮次中位数；全部完成，系统失败为 0。**这是异步确认改造前的基线**，用于追踪演进，不代表当前代码的性能。成功 QPS 包含符合协议的 `NOT_FOUND`，不代表全部命中。
...
线程数对照使用同一份 `25cec55` 二进制，仍属于改造前版本。实验顺序为先 20 线程、后 40 线程；同机负载未完全受控。它支持优化方向，异步实现的收益还需在保持负载、持久化语义和错误检查一致的条件下另行测量。
...
| 优先级 | 方向 | 验收方式 |
| --- | --- | --- |
| P1 | 验证异步确认的效果与边界 | 固定版本与负载进行改造前后对照，联合检查 QPS、尾延迟、请求名额、内存和故障恢复；覆盖慢盘及过载 |
