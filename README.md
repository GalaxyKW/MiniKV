...
查询不触发同步或快照，也不返回 key/value；HTTP 200 只表示状态读取成功，存储健康需看 `engine.io_failed`。计数器随对应进程重启归零，序列从文件恢复。字段及采样口径见[运行状态指南](docs/observability.md)。
...
随后完成了 **18 轮、每轮 500,000 请求的 throughput 旧→新→旧对照**，同样全部通过操作计数和 WAL 排空检查。新版部分轮次吞吐下降、尾延迟扩大；这还不足以确认根因，也不能宣称公共路径没有额外成本。[完整记录](docs/performance-throughput-controls.md)保留了所有慢轮及资源样本，该实验不包含之后对 throughput 路径的进一步简化。
...
| 优先级 | 方向 | 验收条件 |
| --- | --- | --- |
| P1 | 扩大异步确认的负载验证 | 交错版本复测 throughput 路径；延长运行，加入慢盘、过载和固定到达率负载，联合检查失败率、P99、请求名额与内存 |
| P2 | 量化大数据集的快照代价 | 增大 key 数及 value，记录状态复制暂停、峰值内存、WAL 积压与写放大 |
