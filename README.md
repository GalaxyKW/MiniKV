...
- **有界的异步确认**：可靠请求等待 WAL 时释放数据线程；名额覆盖排队、持久化等待与完成处理，断连也不会绕过限制。
...
想了解实现，可从[提交语义](#成功响应意味着什么)和[存储设计](docs/design.md)开始；想评估工程质量，可查看[故障测试](#正确性如何验证)和[附带原始记录的版本对照](docs/performance-async-controls.md)。当前面向单机、全量内存数据集；鉴权、TLS、事务与复制尚未实现。
...
实线展示请求、完成处理与文件写入路径，虚线展示后台协作。快照按周期触发，也可由引擎内部接口调用。

Go 网关负责 HTTP 参数校验、超时和连接复用；C++ 引擎负责请求调度、内存状态与持久化。TCP 使用带长度和状态码的二进制帧；`epoll` 管理连接收发，完整请求才进入工作队列，因此空闲连接和半包不会占住工作线程。

可靠模式将“执行操作”和“等待提交”分开：worker 保存响应及其目标序列后继续工作，完成线程在 WAL 同步后通知 reactor。一个请求的容量名额一直保留到结果被消费或丢弃；同一连接仍按顺序处理请求。具体契约见[异步确认与生命周期](docs/design.md#异步可靠确认与请求生命周期)。
...
响应包含引擎的已应用 / 已持久化序列、未同步 WAL 字节、提交与快照阶段耗时，以及数据任务队列、请求名额和网关 RPC 池状态。等待指标区分请求排队、WAL 容量等待与可靠确认等待。可靠确认已交给完成线程，因此工作线程活跃数不能单独代表在途请求量。查询不触发同步或快照，也不返回 key/value。
...
| 等待可靠确认是否必须占据线程 | 捕获响应与目标序列，提交后由独立完成线程通知 reactor | 需要管理回调生命周期与容量；WAL 容量等待仍会占据 worker |
| 快照期间的新写入如何保留 | 复制序列 S 的状态，持久化快照后原子替换 WAL，保留 S 之后的日志 | 复制数据仍持有状态锁并需要额外内存；重写 WAL 后缀会暂停其他 WAL I/O |
| 半包、过载与断连如何处理 | 收齐帧后派发；为整个请求保留名额，另限制连接、任务队列和 RPC 池 | 满载时等待、返回 BUSY 或关闭新连接；超时不撤销已接纳的操作 |
...
| 异步响应保留可靠语义 | 暂停不同 WAL 批次，验证捕获值、恰好一次回调、容量拒绝、失败与关闭排空 | [async_test.cpp](tests/async_test.cpp) |
...
最新实验比较异步确认版本 `7d29382` 与随后重新运行的同步版本 `25cec55`，两者均使用 **reliable 模式和 20 个数据线程**。每组重复三次，以下列出轮次中位数；共 12 轮全部完成，系统失败为 0。成功 QPS 包含符合协议的 `NOT_FOUND`，不代表全部命中。
...
关闭快照时，活动数据线程的样本均值中位数由同步版的 **19.74 降到 0.55**，平均排队由 **3.268 ms 降到 0.068 ms**。可靠确认仍在等待，工作线程可以继续提交其他请求；新版回调失败为 0，结束后两层请求名额均已释放。完整轮次、等待口径、CPU、内存与原始归档见[异步确认版本对照](docs/performance-async-controls.md)。

这些是共享主机、特定闭环负载下的观测，不是容量承诺。执行顺序没有随机交错，P99 也有波动；本轮只验证 reliable 模式，公共请求路径对 throughput 的额外成本仍待独立比较。更早的 **24 轮[初始基线](docs/performance-baseline.md)** 保留了两种模式和采样开关的记录；它们属于旧实现，不能当作当前版本的性能。

### 从实测问题到设计改动

| 观察与验证 | 得到的依据 | 对应行动 |
| --- | --- | --- |
| 初始基线中，可靠模式数据线程接近占满、任务持续排队 | RPC 池尚有余量，需进一步区分线程到底在等什么 | 增加排队、WAL 容量和可靠确认三类等待指标 |
| 12 轮[刷新间隔 2→1→2 ms 实验](docs/performance-controls.md) | 提交周期与吞吐量随参数改变并返回 | 保留返回对照，继续检查确认等待的线程成本 |
| 12 轮[数据线程 20→40 对照](docs/performance-worker-controls.md) | 无 WAL 容量等待；关闭快照时，QPS 中位数约 5,111→9,399，排队均值约 3.272→0.094 ms | 将可靠确认等待移出数据 worker，同时限制待完成请求数量 |
| 新版与旧版返回对照，仍各用 20 个数据线程 | 可靠确认释放 worker，排队下降；回调与请求名额正常排空 | 保留语义及故障回归，继续检查其他模式和更大负载的代价 |

线程数对照使用同一份 `25cec55` 二进制，属于改造前的机制验证；随后才实现并测量 `7d29382`。各份报告分别保留执行版本、配置和完整轮次，避免把不同实验的差值混为同一种收益。
...
输出目录必须尚不存在。默认运行 4 种配置、每种 3 次，每轮 20,000 请求、20 个客户端 worker、1,000 key、4 个引擎线程；这与上表的负载不同。重跑上表配置见[版本对照复现步骤](docs/performance-async-controls.md#原始归档与复现)。
...
已经完成“基线 → 等待指标 → 单变量对照 → 异步确认”的一轮演进。接下来优先验证改造在不同负载下的实际收益和代价，再考虑扩大存储能力。

| 优先级 | 方向 | 验收方式 |
| --- | --- | --- |
| P1 | 扩大异步确认的验证范围 | 对比 throughput 公共请求路径成本，增加负载时长，联合检查 QPS、尾延迟、请求名额与内存；覆盖慢盘及过载 |
...
| 最新版本对照、资源开销与原始数据 | [异步确认性能实验](docs/performance-async-controls.md) |
| 两种模式与采样开关的初始记录 | [初始性能基线](docs/performance-baseline.md) |
| 单变量参数比较与返回对照 | [WAL 刷新间隔控制实验](docs/performance-controls.md) |
| 排队、线程占用与异步化依据 | [数据线程数控制实验](docs/performance-worker-controls.md) |
